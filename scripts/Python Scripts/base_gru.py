...
for year in range(2011, 2018 + 1):
    ...
    for FUTURE_STEPS in future_steps_list:
        ...
        model = Sequential([
            Bidirectional(GRU(260, activation='relu', return_sequences=True, input_shape=(X.shape[1], X.shape[2]))),
            Dropout(0.15),
            Bidirectional(GRU(248)),
            Dropout(0.05),
        ...
        plt.title(f"GRU Model Predictions vs Actual (First Future Step, {FUTURE_STEPS} Future Steps, Year {year})")
        plt.xlabel("Index")
        plt.ylabel("White_Spot_Percentage")
        plt.legend()

        # Save the plot as an image
        plt.savefig(f'/home/tarek/summer_2024/original_gru/graphs/gru_predictions_{year}_lookback{TIME_STEPS}_futuresteps{FUTURE_STEPS}_batch_size_12_epochs_30.png')
        plt.close()

# Save all results to a CSV file
results_df = pd.DataFrame(all_results)
results_df.to_csv('/home/tarek/summer_2024/original_gru/gru_results_48_24_batch_size_12_epochs_30.csv', index=False)
